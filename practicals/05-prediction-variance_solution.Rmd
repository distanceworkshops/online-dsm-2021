...
However, let's remember that this was not our favoured model previously and remains so: the negative binomial model's AIC is considerably better

```{r tw-nb-AIC}
AIC(dsm_nb_xy_ms_refit, dsm_tw_xy_ms_refit)
```

And the quantile-quantile plot shows the negative binomial model has a better fit:
...
Again the Tweedie model here is just for example, we didn't prefer this model based on AIC or quantile-quantile plots.
