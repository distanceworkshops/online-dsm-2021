...
First checking that `k` is big enough, we should really do this during model fitting, but we've separate this up for the pratical exercises.
...
Continuing with that exmaple, if we double `k`:
...
Now save the models that you'd like to use to precict with later (that have good check results!): I recommend saving a few models so you can compare the results later.
